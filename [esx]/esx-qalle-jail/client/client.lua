...
function JailLogin()
	...
	ESX.ShowNotification("La dernière fois que vous vous êtes endormi, vous avez été emprisonné, vous êtes maintenant de retour!")
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Vous avez " .. jailTime .. " minutes restantes en prison!")
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Lacher le paquet", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Emprisonner le plus proche", value = "jail_closest_player" },
				{ label = "Relacher un prisonnier", value = "unjail_player" }
			}
		}, 
	function(data, menu)
		...
		if action == "jail_closest_player" then
			...
			ESX.UI.Menu.Open(
          		...
          		{
            		title = "Temps d'emprisonnement (minutes)"
          		},
            	...
            	if jailTime == nil then
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("VOus devez remplir ceci!")
						  	else
								menu3.close()
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	ESX.ShowNotification("Personne à côté !")
								else
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Prisonnier: " .. playerArray[i].name .. " | Temps d'emprisonnement: " .. playerArray[i].jailTime .. " minutes", value = playerArray[i].identifier })
				end

				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'jail_unjail_menu',
					{
						title = "Libérer prisonnier",
						align = "center",
						elements = elements
					},
				function(data2, menu2)
