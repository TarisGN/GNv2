    ...
    ESX.RegisterUsableItem(item_name, function(source)
        ...
        if cops >= options[item_name].cops then
            ...
            TriggerClientEvent("pNotify:SendNotification", _source, {
                text = 'Pour démarrer cette opération, doit être au moins '.. options[item_name].cops .. ' policiers.',
                type = "error",
                timeout = 5000,
                layout = "centerLeft"
            })
            TriggerClientEvent("pNotify:SendNotification", _source, {
                text = 'Nombre actuel de policiers en service: '.. cops ..'/'.. options[item_name].cops,
                type = "info",
                timeout = 6000,
                layout = "centerLeft"
            })
