...
if stage7 == true then
Draw3DText(plyPos.x, plyPos.y, plyPos.z, tostring("~r~C'est le message de test 7"))
end
		Citizen.Wait(0)
	end
end)
...
--	TriggerEvent('chatMessage', 'Bilvasken', {255, 0, 0}, "Du har ikke penge nok, du mangler ^1kr" .. moneyleft .. "") -- danish
	TriggerEvent('chatMessage', 'CivRepair', {255, 0, 0}, "VOus n'avez pas assez, il vous manque ^1$" .. moneyleft .. "") 
	DrawSpecialText(msg, 5000)
	Wait(5000)
end)
    --[[ 
...
--	TriggerEvent('chatMessage', 'Bilvasken', {255, 0, 0}, "Du har ikke penge nok, du mangler ^1kr" .. moneyleft .. "") -- danish
	TriggerEvent('chatMessage', 'CivRepair', {255, 0, 0}, "Vous 'navez pas assez pour le ^3Premium^0 il vous manque ^1$" .. moneyleft .. "") 
	DrawSpecialText(msg, 5000)
	Wait(5000)
end)
...
function AutoRepair() -- Credits qalle for assisting with this 
    ...
    ESX.UI.Menu.Open(
        ...
        {
            title    = "<font color='lightblue' font-size='15px'>🔧AUTO REPAIR🔧</font>",
            align    = 'center',
            elements = elements
        },
    function(data, menu)
