	...
	if DoesEntityExist(vehicle) then
		if IsPedInAnyVehicle(player) or IsPedInAnyHeli(player) then
				...
				if not table.empty(VehicleList) then
					...
					if isvehiclefound then
						if (islocked == 1)then
							...
							Notify("Le véhicule ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~ est ~r~vérouillé ~w~.")
							-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
						else
							SetVehicleDoorsLocked(vehicle,1)
							Notify("Le véhicule ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~ est ~g~dévérouillé ~w~.")
			...
			if not table.empty(VehicleList) then
					...
					if isvehiclefound then
						if not foundclostestvehicle then 
							if (islockedHandle == 1)then
								...
								Notify("Le véhicule ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleHandle))) .. "~w~ est ~r~vérouillé ~w~.")
								-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
							else
								SetVehicleDoorsLocked(vehicleHandle,1)
								Notify("Le véhicule ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleHandle))) .. "~w~ est ~g~dévérouillé ~w~.")
								-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "unlock", 1.0)
							end
						else
							if (islockedclostestvehicle == 1)then
								SetVehicleDoorsLocked(clostestvehicle, 2)
								Notify("Le véhicule ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(clostestvehicle))) .. "~w~ est ~r~vérouillé ~w~.")
								-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
								foundclostestvehicle = false
							else
								SetVehicleDoorsLocked(clostestvehicle,1)
								Notify("Le véhicule ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(clostestvehicle))) .. "~w~ est ~g~dévérouillé ~w~.")
								-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "unlock", 1.0)
								foundclostestvehicle = false
							end
				...
				Notify("~r~Pas de véhicule à vérouiller, ou le véhicule est vérouillé.")
			end
		end
	else
