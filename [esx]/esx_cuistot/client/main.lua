...
function OpenCloakroomMenu()
  ...
  local elements = {
    { label = "Sua roupa",     value = 'citizen_wear'},
    { label = "Empregado",    value = 'barman_outfit'}
  }
...
function OpenVaultMenu()
    ...
    local elements = {
      {label = "Ovo - 1€", value = 'ovo'},
      {label = "Vitela Crua - 17€", value = 'vitela'},
      {label = "Polvo Cru - 18€", value = 'polvo'},
      {label = "Alho - 2€", value = 'alho'},
      {label = "Alface - 3€", value = 'alface'},
      {label = "Tomate - 4€", value = 'tomate'},
      {label = "Oregaos - 2€", value = 'oregaos'},
      {label = "Carne Picada - 7€", value = 'carnep'},
      {label = "Pao - 1€", value = 'bread'},
      {label = "Agua - 1€", value = 'water'},
      {label = "Queijo - 5€", value = 'queijo'},
      {label = "Manteiga - 3€", value = 'manteiga'}
    }
    ...
    ESX.UI.Menu.Open(
	  ...
	  local valor = 0
		...
		if data.current.value == "ovo" then
			valor = 1
		end
		
		if data.current.value == "vitela" then
			valor = 17
		end
		...
		if data.current.value == "alho" then
			valor = 2
		end
		
		if data.current.value == "alface" then
			valor = 3
		end
		...
		if data.current.value == "oregaos" then
			valor = 2
		end
		
		if data.current.value == "manteiga" then
			valor = 3
		end
		
		if data.current.value == "carnep" then
			valor = 7
		end
		...
		if data.current.value == "queijo" then
			valor = 5
		end
		TriggerServerEvent('esx_cozinheirojob:shop', data.current.value, valor)
		TriggerServerEvent('esx_society:depositMoney', "gouvernor", valor)
      end,
...
function OpenFridgeMenu()
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        CurrentActionMsg  = "~g~E~w~ Para abrir o frigorifico"
        CurrentActionData = {}
      end
    )
...
function OpenCozinharMenu()
	...
	local elements = {
      {label = "Ovos estrelados", value = 'ovoes'},
	  {label = "Hamburguer", value = 'ham'},
      {label = "Polvo Grelhado", value = 'polvo'},
      {label = "Vitela assada", value = 'vitela'}
    }
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        CurrentActionMsg  = "~g~E~w~ Para cozinhar algo"
        CurrentActionData = {}
      end
    )
...
AddEventHandler('esx_cozinheirojob:hasEnteredMarker', function(zone)
	...
	if zone == 'Cook' then
      CurrentAction     = 'menu_cook'
      CurrentActionMsg  = "~g~E~w~ para cozinhar."
      CurrentActionData = {}
    end
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(0,  Keys['F7']) and IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'cozinheiro_actions') then
