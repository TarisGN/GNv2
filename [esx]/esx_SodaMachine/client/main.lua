...
function OpenShopMenu(zone)
	local elements = {}
	TriggerServerEvent('esx_SodaMachine:buyItem', "soda", 1)
end
...
AddEventHandler('esx_SodaMachine:Random', function(prop_name)
	...
	if lottery <= 6 then
		if not IsAnimated then
			...
			Citizen.CreateThread(function()
				...
				ESX.ShowNotification('En cours de préparation. Attendez...')
				TriggerServerEvent('esx_SodaMachine:TakeMoney', 100)
				Citizen.Wait(3000)
				...
				Citizen.Wait(700)
				ESX.ShowNotification('Vous achetez une boisson. Merci!')
	...
	if lottery == 7 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('EN PANNE')
			end)
		end
	if lottery == 8 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('Plus de produits')
			end)
		end
	if lottery == 9 then
			Citizen.CreateThread(function()
			...
			TriggerEvent('InteractSound_CL:PlayOnOne', 'SodaMachine', 1.0)
			Citizen.Wait(5000)
			ESX.ShowNotification('Il y a eu un probleme, la machine a pris votre argent sans vous donner la boisson')
			FreezeEntityPosition(ped, false)
			end)
		end
end)